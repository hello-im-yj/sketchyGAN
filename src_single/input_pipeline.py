...
# sketchy_dir = '../rendered_256x256/256x256/photo/tx_000000000000'
# flickr_dir = '../rendered_256x256/256x256/sketch/tx_000000000000'

# in Colab
sketchy_dir = '../../../content/drive/MyDrive/그림팀/rendered_256x256/256x256/photo/tx_000000000000'
flickr_dir = '../../..//content/drive/MyDrive/그림팀/rendered_256x256/256x256/sketch/tx_000000000000'
...
paired_filenames_1 = []
for f in os.listdir(sketchy_dir) :
    if os.path.isdir(os.path.join(sketchy_dir, f)) :
        for img_file in Path(os.path.join(sketchy_dir, f)).iterdir() : 
            paired_filenames_1.append(str(img_file))
paired_filenames_2 = []
...
for f in paired_filenames_1:
    # id = os.path.splitext(os.path.split(f)[1])[0].split('_')[0]
    name = f.split('/')[-2]
    class_id = classes_id.index(name)
    if class_id not in class_mapping:
        class_mapping.append(class_id)
class_mapping = sorted(class_mapping)
for f in paired_filenames_2:
    # id = os.path.splitext(os.path.split(f)[1])[0].split('_')[0]
    name = f.split('/')[-2]
    class_id = classes_id.index(name)
    if class_id not in class_mapping:
